...
Live Coinbase path is **`coinbase_onchain`** (CDP / Base wallet) only, and only when guardian `CDP_API_KEY_ID` / `CDP_API_KEY_SECRET` / `CDP_WALLET_SECRET` **or** `COINBASE_CDP_*` (or the documented old-guide aliases) are present. Shared Railway: no remapping. See [COINBASE_LIVE_SWITCH.md](../COINBASE_LIVE_SWITCH.md). WAVE hitch USD quotes use GeckoTerminal then DexScreener and fail closed if price is missing.
